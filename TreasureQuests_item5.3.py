...
starts_rect = pygame.Rect(300, 200, 150, 50)
...
while runtime:
    ...
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.type == pygame.QUIT:
                runtime = False
            elif scene_menu == "menu"and starts_rect.collidepoint(event.pos):
                scene_menu = "play"
    ...
    if scene_menu == "play":
        ...
        if man_rect.colliderect(square_rect):
            velocity_y = 0
        if scene_menu == "menu":
            pygame.draw.rect(screen, green, starts_rect)
        elif scene_menu == "play":
            #sprite
            if moving:
                man_image = man_standing
            else:
